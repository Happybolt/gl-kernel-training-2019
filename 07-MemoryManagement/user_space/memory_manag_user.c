...
static int32_t get_timeout(int32_t start_time, int32_t end_time)
{
	return start_time < end_time ? end_time - start_time :
					(1000000000 - start_time + end_time);
}
...
static void mem_test(struct mem_description *descr)
{
	...
	int32_t time_spent_allocate;
	int32_t time_spent_free;
	...
	while (size < descr->config.max_size_mem) {
		...
		time_spent_allocate = get_timeout(ts_start.tv_nsec,
				ts_end.tv_nsec);
		...
		if (res_allocate != NULL &&
			...
			clock_gettime(CLOCK_REALTIME, &ts_end);
			time_spent_free = get_timeout(ts_start.tv_nsec,
					ts_end.tv_nsec);
		} else {
			time_spent_free = 0;
		}
